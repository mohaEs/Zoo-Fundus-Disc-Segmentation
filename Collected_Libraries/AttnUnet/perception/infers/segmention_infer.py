# -- coding: utf-8 --
"""
Copyright (c) 2018. All rights reserved.
Created by Resnick Xing on 2018/5/11
"""
import glob,cv2,numpy as np
import matplotlib.pyplot as plt
from tensorflow.keras.models import model_from_json
from Collected_Libraries.AttnUnet.perception.bases.infer_base import InferBase
from Collected_Libraries.AttnUnet.configs.utils.img_utils import get_test_patches,pred_to_patches,recompone_overlap,imgResize,postprocess
from Collected_Libraries.AttnUnet.configs.utils.utils import visualize,gray2binary
import os

class SegmentionInfer(InferBase):
	def __init__(self, config, ImagesDIR, savepath):
		super(SegmentionInfer, self).__init__(config)
		self.load_model()
		self.ImagesDIR=ImagesDIR
		self.savepath=savepath

	def load_model(self):
		self.model = model_from_json(open('./Collected_Libraries/AttnUnet/hdf5/OpticDisc_architecture.json').read())
		self.model.load_weights('./Collected_Libraries/AttnUnet/hdf5/OpticDisc_best_weights.h5')

	def analyze_name(self,path):
		return (path.split('\\')[-1]).split(".")[0]

	def predict(self):
		predList=glob.glob(os.path.join(self.ImagesDIR, "*"))
		for path in predList:
			orgImg_temp=plt.imread(path)
			if orgImg_temp.shape[0]>1000 or orgImg_temp.shape[1]>1000:
				orgImg_temp=imgResize(orgImg_temp,0.5)
			orgImg=orgImg_temp[:,:,1]*0.75+orgImg_temp[:,:,0]*0.25
			print("[Info] Analyze filename...",self.analyze_name(path))
			height,width=orgImg.shape[:2]
			orgImg = np.reshape(orgImg, (height,width,1))
			patches_pred,new_height,new_width,adjustImg=get_test_patches(orgImg,self.config)

			predictions = self.model.predict(patches_pred, batch_size=32, verbose=1)
			pred_patches=pred_to_patches(predictions,self.config)

			pred_imgs=recompone_overlap(pred_patches,self.config,new_height,new_width)
			pred_imgs=pred_imgs[:,0:height,0:width,:]


			probResult=pred_imgs[0,:,:,0]
			binaryResult=gray2binary(probResult,threshold=0.5)

			binaryResult,probResult=postprocess(binaryResult,probResult) # post process to get the optic disc


			_, contours, _ = cv2.findContours(binaryResult, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
			adjustImg = cv2.drawContours(orgImg_temp, contours, -1, (0, 255, 0), 2)

			resultMerge=visualize([adjustImg/255.,binaryResult],[1,2])
			resultMerge=cv2.cvtColor(resultMerge,cv2.COLOR_RGB2BGR)
			
		
			cv2.imwrite(os.path.join( self.savepath, self.analyze_name(path) + ".jpg"), (probResult*255).astype(np.uint8))


